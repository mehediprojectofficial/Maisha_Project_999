...
module.exports = {
  ...
  // ─────────────────────────────────────────────────────────────────
  onReply: async function ({ api, event, Reply, message, globalData, threadsData }) {
    const { author, state } = Reply;
    if (event.senderID !== author) return;

    const configPath = path.join(process.cwd(), "config.json");
    // Use the live in-memory config (global.GoatBot.config) instead of a
    // fresh JSON.parse of the file. Handlers (antiSpam, noPrefix, maintenance
    // mode, etc.) all read from global.GoatBot.config — editing a separately
    // parsed copy only changed the file on disk and never took effect until
    // a full bot restart. Mutating the same object fixes both file + live.
    ...
    if (state === "main") {
      if (num === 1) {
        const threadOnlyAdminBox = await threadsData.get(event.threadID, "data.onlyAdminBox", false);
        return await sendAndListen(
          [
            ...
            `4. Only Admin Box (this group) — ${status(threadOnlyAdminBox)}`,
            `5. Anti Spam — ${status(config.antiSpam?.enable)}`,
            `6. Command Performance Logging — ${status(config.commandPerformance?.enable)}`,
            `7. Maintenance Mode — ${status(config.maintenanceMode?.enable)}`,
            `8. No Prefix — ${status(config.noPrefix?.enable)}`,
            "━━━━━━━━━━━━━━━━━",
            "› Reply 1-8 to toggle",
          ].join("\n"),
          "botConfig"
    ...
    if (state === "botConfig") {
      if (num === 1) { config.adminOnly = config.adminOnly || {}; config.adminOnly.enable = !config.adminOnly.enable; }
        ...
        // Per-thread flag stored via threadsData (matches the {prefix}onlyadminbox
        // command) — config.onlyAdminBox doesn't exist anywhere and never did
        // anything; the handler actually checks threadData.data.onlyAdminBox.
        const current = await threadsData.get(event.threadID, "data.onlyAdminBox", false);
        await threadsData.set(event.threadID, !current, "data.onlyAdminBox");
        return message.reply(`✦ Only Admin Box (this group) — ${status(!current)}`);
      }
      else if (num === 5) { config.antiSpam = config.antiSpam || {}; config.antiSpam.enable = !config.antiSpam.enable; }
      else if (num === 6) { config.commandPerformance = config.commandPerformance || {}; config.commandPerformance.enable = !config.commandPerformance.enable; }
      else if (num === 7) { config.maintenanceMode = config.maintenanceMode || {}; config.maintenanceMode.enable = !config.maintenanceMode.enable; }
      else if (num === 8) { config.noPrefix = config.noPrefix || {}; config.noPrefix.enable = !config.noPrefix.enable; }
